function download {
    ...
    try {
      ...
      if ($statusCode -eq 401 -and $SuccessOn401.IsPresent) {
        return $true
      } else {
        if ($statusCode -eq $null) {
          Write-Host "status code is null"
        }else {
...
function post_build_checks {
    ...
    if ((Get-NetAdapter | where {$_.name -eq "VMware Network Adapter VMnet2"}).ifIndex) {
      ...
      if ((get-netipaddress -AddressFamily IPv4 | where { $_.InterfaceIndex -eq $vmnet2idx }).IPAddress -ne "192.168.56.1") {
        Write-Host "'[!] Your vmnet2 network adapter is ' + $((get-netipaddress -AddressFamily IPv4 | where { $_.InterfaceIndex -eq $vmnet2idx }).IPAddress)"
